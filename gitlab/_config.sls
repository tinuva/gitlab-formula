...
gitlab:
  git:
    ...
    - rev: {{ salt['pillar.get']('gitlab:version', '7-9-stable') }}
    - user: {{ gl_user }}
...
shell_setup:
  cmd:
    - wait
    - name: bundle exec rake gitlab:shell:install[{{ salt['pillar.get']('gitlab:shell:version', 'v2.6.0') }}] REDIS_URL=redis://localhost:6379 RAILS_ENV=production
    - user: {{ gl_user }}
    - cwd: {{ gl_home }}/gitlab
